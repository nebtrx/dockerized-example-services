...
import com.github.gvolpe.fs2rabbit.model.{AmqpMessage, AmqpProperties, ExchangeType, StreamPublisher}
...
class RabbitProducer[F[_]: Concurrent: Timer](implicit F: Fs2Rabbit[F],
                                                       SE: StreamEval[F],
  ...
  private def flowTo(publisher: StreamPublisher[F]): Stream[F, Unit] = {
    ...
    val dataStream: Stream[F, AmqpMessage[Tweet]] = TWStream[F]
      .stream
      .map(t => AmqpMessage(t, AmqpProperties.empty))

    val throttlerStream = Stream.awakeEvery[F](1.second)

    val finalStream: Stream[F, AmqpMessage[Tweet]] = dataStream.zipWith(throttlerStream)((v, _) => v)
...
object RabbitProducer {
  def apply[F[_]: Concurrent: Timer](implicit F: Fs2Rabbit[F],
                                              SE: StreamEval[F],
                                              CE: ConcurrentEffect[F],
                                              CS: ContextShift[F],
                                              A: ApplicativeAsk[F, AppConfig])
